...
def _conductivity_model_1(lamb_g, lamb_f, porosity):
    ...
    """
    if lamb_g == 0:
        raise ValueError("λ_g must be non-zero to compute conductivity model 1.")
...
def _conductivity_model_2(lamb_g, lamb_f, porosity):
    ...
    """
    if lamb_g == 0:
        raise ValueError("λ_g must be non-zero to compute conductivity model 2.")
    r = lamb_f / lamb_g
    phi = porosity
...
def weighted_parameter(model, ground_parameter, fluid_parameter, porosity):
    ...
    print(f"Effective volumetric heat capacity (ρc_eff): {rho_c_eff:.2f} J/(m³·K)")
...
def P_el_values(Q: float, T : float, T_H: float, delta_t: float, gamma: float):
    ...
    try:
        if Q < 0 and T < T_H :
            W_el = Q * delta_t * (1 - T / T_H) / gamma / 3600 # delta_t in s; Q in W/m --> W_el in Wh/m 
